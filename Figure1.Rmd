...
# Figure 1G - gene correlation 

# 1. Preparation
see Figure 1F -> diet_data_ileum

```{r}
table(diet_data_ileum$diet)
```
...
```{r}
cor.test(diet_data_ileum$Microbial_Load, diet_data_ileum$CCL5, method = "spearman")
```
...
```{r}
diet_colors <- c("#85C1E9", "#AF7AC5", "#F39C12", "#52BE80")
diet_labels <- c("control"= "RCD", "fasting" = "Fasting", "high_fat" = "HFaD", "high_fiber"= "HFiD")

pdf("Diet_ileum_CCL5_load_corr.pdf", width = 10, height = 7)
...
ggplot(aes(Microbial_Load, CCL5)) +
  ...
  scale_color_manual(values = diet_colors,  name = "Diet", labels = diet_labels) +
  scale_fill_manual(values = diet_colors,  name = "Diet", labels = diet_labels) +
