...
## 1. Preparation
excluding mouse 529 due to colitis symptoms
```{r}
...
ggplot(
  plot_data_TRF,
  aes(x = Gene, y = Expression, fill = diet_genotype, color = diet_genotype)) +
  geom_boxplot(lwd = 1.2, width = 0.6, outlier.shape = NA, alpha = 0.2, position = pos) +
  geom_point(size = 2, alpha = 0.7, position = pos) +
  scale_fill_manual(
    values = diet_colors,
    labels = c(
