...
This scripts includes all necessary preparations to generate data frames for the analyses and figures in the manuscript. 
Input files for this script can be found in the supplemental information of the manuscript in Zenodo
...
Settings_line <- theme_classic(base_size = 30) + # Fig. 4 ggline GI Emptying plots 
...
Settings_barplot <- theme_classic(base_size = 30) + # variance barplot (Fig. 5) and EMM summary plot (Fig. 4)
  theme(#legend.position= "none",
        #legend.text = element_text(hjust = 0.5),
        #legend.title = element_blank(),  
        #aspect.ratio = 1,
...
Settings_corr_all <- theme_classic(base_size = 20) + 
...
## 2.1 Metadata
```{r}
metadata_QP_interval <- read.table("metadata_QP_interval.txt", sep = '\t', comment = '', head=T) 
```
...
```{r}
# Diet
ASV_count_Diet <- read.table("ASV_count_Diet.txt", sep = '\t', comment = '', head=T, row.names = 1, check.names = F) 

# TRF 
ASV_count_TRF <- read.table("ASV_count_TRF.txt", sep = '\t', comment = '', head=T, row.names = 1, check.names = F) 

# combined
ASV_count_combined <- read.table("ASV_count_combined.txt", sep = '\t', comment = '', head=T, row.names = 1, check.names = F) 
```
...
```{r}
# Diet
L6_count_Diet <- read.table("L6_count_Diet.txt", sep = '\t', comment = '', head=T, row.names = 1, check.names = F) 

# TRF 
L6_count_TRF <- read.table("L6_count_TRF.txt", sep = '\t', comment = '', head=T, row.names = 1, check.names = F) 

# combined
L6_count_combined <- read.table("L6_count_combined.txt", sep = '\t', comment = '', head=T, row.names = 1, check.names = F) 
```

Phylum (L2)
```{r}
# combined
L2_count_combined <- read.table("L2_count_combined.txt", sep = '\t', comment = '', head=T, row.names = 1, check.names = F)
```
...
```{r}
weight_diet <- read.table("weight_diet.txt", sep = '\t', comment = '', head=T, row.names = 1, check.names = F) 
...
```{r}
metadata_QP_passage <- read.table("metadata_QP_passage.txt", sep = '\t', comment = '', head=T)
```
...
**Diet**
```{r}
L6_relAb_Diet <- sweep(L6_count_Diet, 1, rowSums(L6_count_Diet),'/') # 

# merge with metadata
L6_relAb_Diet <- L6_relAb_Diet %>% rownames_to_column("sampleID")
meta_L6_relAb_Diet <- metadata_QP_interval %>% filter(project == "diet_study") %>% left_join(L6_relAb_Diet, by = "sampleID") # = input for per interval GLMMs (Suppl. Figure GLMMs)
```
...
meta_L6_relAb_TRF <- metadata_QP_interval %>% filter(project == "TRF_study") %>% left_join(L6_relAb_TRF, by = "sampleID") # input for per interval GLMMs (Suppl. Figure GLMMs)
```

**combined L6**
```{r}
L6_relAb_combined <- sweep(L6_count_combined, 1, rowSums(L6_count_combined),'/') # 

# merged with metadata
L6_relAb_combined <- L6_relAb_combined %>% rownames_to_column("sampleID")
meta_L6_relAb_combined <- left_join(metadata_QP_interval, L6_relAb_combined, by = "sampleID") 
```

**combined L2**
```{r}
L2_relAb_combined <- sweep(L2_count_combined, 1, rowSums(L2_count_combined),'/') 

# merged with metadata
L2_relAb_combined <- L2_relAb_combined %>% rownames_to_column("sampleID")
meta_L2_relAb_combined <- left_join(metadata_QP_interval, L2_relAb_combined, by = "sampleID")  
```
...
meta_ASV_Diet <- metadata_QP_interval %>% filter(project == "diet_study") %>% left_join(ASV_count_Diet, by = "sampleID")  
```
...
meta_ASV_TRF <- metadata_QP_interval %>% filter(project == "TRF_study") %>% left_join(ASV_count_TRF, by = "sampleID")    
```
...
meta_ASV_combined <- left_join(metadata_QP_interval, ASV_count_combined, by = "sampleID")  
```
...
for Supplementary Figure GLMM Fig.5
